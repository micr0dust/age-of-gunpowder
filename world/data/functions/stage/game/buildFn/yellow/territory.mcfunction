...
execute @e[tag=cmd,c=1,score_yfield_gun_min=1] ~ ~ ~ tellraw @p[team=yellow] ["",{"text":"[","color":"yellow"},{"translate":"aop.cannon","clickEvent":{"action":"run_command","value":"/trigger building set 9"},"hoverEvent":{"action":"show_text","value":[{"translate":"aop.cannon.hover"}]}},{"text":"] ","color":"yellow"},{"text":"250","color":"gold"}]
