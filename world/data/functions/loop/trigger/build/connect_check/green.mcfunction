...
execute @e[tag=green,score_connect_min=1,score_connect=1,name=!wall] ~ ~ ~ execute @e[tag=building,score_connect=0,r=9] ~ ~ ~ execute @e[tag=green,r=0,c=1] ~ ~ ~ scoreboard players set @e[tag=cmd,c=1] b 1
function loop:trigger/build/connect_check/green if @e[tag=cmd,c=1,score_b_min=1]
function loop:trigger/build/connect_check/count/green if @e[tag=cmd,c=1,score_b=0]
