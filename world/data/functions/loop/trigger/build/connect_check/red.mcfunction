...
execute @e[tag=red,score_connect_min=1,score_connect=1,name=!wall] ~ ~ ~ execute @e[tag=building,score_connect=0,r=9] ~ ~ ~ execute @e[tag=red,r=0,c=1] ~ ~ ~ scoreboard players set @e[tag=cmd] b 1
function loop:trigger/build/connect_check/red if @e[tag=cmd,score_b_min=1]
function loop:trigger/build/connect_check/count/red if @e[tag=cmd,score_b=0]
