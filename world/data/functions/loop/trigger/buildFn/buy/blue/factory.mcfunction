...
#膛線工藝技術
execute @e[tag=cmd,score_bflintlock_min=1,score_brifling=0] ~ ~ ~ execute @p[score_buildFn_min=51,score_buildFn=51,team=blue] ~ ~ ~ function loop:trigger/build/buy/noresoure unless @e[tag=cmd,score_bgold_min=500]
execute @e[tag=cmd,score_bflintlock_min=1,score_brifling=0] ~ ~ ~ execute @p[score_buildFn_min=51,score_buildFn=51,team=blue] ~ ~ ~ function loop:trigger/buildfn/buy/cost/blue/rifling if @e[tag=cmd,score_bgold_min=500]
...
#簧輪式點火
execute @e[tag=cmd,score_bwheellock=0] ~ ~ ~ execute @p[score_buildFn_min=51,score_buildFn=51,team=blue] ~ ~ ~ function loop:trigger/build/buy/noresoure unless @e[tag=cmd,score_bgold_min=500]
execute @e[tag=cmd,score_bwheellock=0] ~ ~ ~ execute @p[score_buildFn_min=51,score_buildFn=51,team=blue] ~ ~ ~ function loop:trigger/buildfn/buy/cost/blue/wheellock if @e[tag=cmd,score_bgold_min=500]
