...
#搶木金
execute @e[name=commandcenter,tag=red,c=1] ~ ~ ~ execute @e[tag=ctrl,score_buildType=0,score_resourceType_min=1,score_resourceType=2,r=54,c=1] ~ ~ ~ function loop:ai/red/wood_gold if @e[tag=cmd,score_rmanpower_ef_min=27,score_rmanpower_min=500]
#人口計算
...
#買房
function loop:ai/red/house if @e[tag=cmd,score_rwood_min=100,score_i_min=-4,score_rmanpower_ef_min=30]
...
#餘額買兵
execute @e[name=commandcenter,score_AI_r=-1,c=1] ~ ~ ~ execute @e[name=barracks,tag=red,c=1,score_connect_min=1] ~ ~ ~ function loop:ai/red/cost/spearman if @e[tag=cmd,score_rmanpower_min=80,score_rwood_min=10,score_i=-1,score_rflintlock=0,score_k=2]
#餘額買兵
execute @e[name=commandcenter,score_AI_r=-1,c=1] ~ ~ ~ execute @e[name=barracks,tag=red,c=1,score_connect_min=1] ~ ~ ~ function loop:ai/red/cost/matchlock if @e[tag=cmd,score_rmanpower_min=80,score_rgold_min=10,score_i=-1,score_rflintlock=0,score_k=2]
#餘額買兵
execute @e[name=commandcenter,score_AI_r=-1,c=1] ~ ~ ~ execute @e[name=barracks,tag=red,c=1,score_connect_min=1] ~ ~ ~ function loop:ai/red/cost/musket if @e[tag=cmd,score_rmanpower_min=80,score_rgold_min=10,score_i=-1,score_rflintlock_min=1,score_k=2]
#餘額買兵
...
#餘額買房
#function loop:ai/red/init_house if @e[tag=cmd,score_rwood_min=500,score_rmanpower_ef_min=100]
#餘額買指揮中心
#function loop:ai/red/commandcenter if @e[tag=cmd,score_rwood_min=400,score_rmanpower_ef_min=150]
#餘額升級
...
#投降
execute @e[tag=cmd,score_rmanpower_ef=12] ~ ~ ~ execute @e[name=commandcenter,tag=red] ~ ~ ~ function @@@:suicide if @e[tag=!red,type=vindication_illager,r=33]
execute @e[name=commandcenter,tag=red] ~ ~ ~ function @@@:suicide if @e[tag=!red,type=shulker,r=40]
